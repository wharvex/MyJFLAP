    ...
    while (it.hasNext()) {
      ...
      for (int i = 0; i < ts.length; i++) {
        ...
        Transition toBeAdded =
            (Transition) ts[i].clone(); //call clone instead of copy so that the gui stuff can get appropriately updated
...
//		}
  ...
  public State setInitialState(State initialState) {
    ...
    distributeStateEvent(
        new AutomataStateEvent(this, initialState, false, false,
            true));
  ...
  public void addFinalState(State finalState) {
    ...
    distributeStateEvent(
        new AutomataStateEvent(this, finalState, false, false,
            true));
  }
...
//        EDebug.print("The Hash is that is hashed, is truly hashed");
    ...
    ret += initialState == null ? 0 :
        (int) (initialState.specialHash() * Math.PI);
