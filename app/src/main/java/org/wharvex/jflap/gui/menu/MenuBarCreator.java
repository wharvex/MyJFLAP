    ...
    if (Universe.CHOOSER != null) {
      ...
      if (environment instanceof AutomatonEnvironment) { //this is strictly for non-Grammar
        ...
        if (new File("svg.jar").exists() ||
          ...
          try {
            RestrictedAction ra =
                (RestrictedAction) Class.forName("gui.action.ExportAction")
...
//					(gui.environment.GrammarEnvironment) environment));
    ...
    if (LSystemDisplay.isApplicable(object))
      addItem(menu, new LSystemDisplay(
          (LSystemEnvironment) environment));
  ...
  private static JMenu getViewMenu(EnvironmentFrame frame) {
    ...
    if (AutomatonAction.isApplicable(object)) {
      ...
      LayoutStorageAction store =
          new LayoutStorageAction("Save Current Graph Layout",
      ...
      subMenu.add(new LayoutAlgorithmAction("Horizontal Line Through Center",
          automaton,
          environment, VertexMover.HORIZONTAL_CENTER));
      subMenu.add(new LayoutAlgorithmAction("Vertical Line Through Center",
          automaton,
      ...
      viewMenu.add(
          new LayoutAlgorithmAction("Fill Screen With Graph", automaton,
              environment, VertexMover.FILL));
      menu.add(viewMenu);

      menu.add(new LayoutAlgorithmAction("Apply A Random Layout Algorithm",
          automaton,
...
//			menu.add(new StateColorSelector(automaton,environment,menu));
  ...
  private static JMenu getTestMenu(EnvironmentFrame frame) {
    ...
    if (GrammarTypeTestAction.isApplicable(object))
      addItem(menu,
          new GrammarTypeTestAction((GrammarEnvironment) environment));
    return menu;
  }
