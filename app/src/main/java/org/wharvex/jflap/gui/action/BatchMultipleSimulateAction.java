...
import org.wharvex.jflap.gui.environment.EnvironmentFactory;
...
import org.wharvex.jflap.gui.environment.EnvironmentFactory.EditorPermanentTag;
...
import org.wharvex.jflap.gui.grammar.parse.UnrestrictedTreePanel;
...
public class BatchMultipleSimulateAction extends MultipleSimulateAction {
   ...
   */
  public BatchMultipleSimulateAction(Automaton automaton,
  ...
  public void performAction(Component source) {
    ...
    if (getObject() instanceof MealyMachine) {
      ...
      NondeterminismDetector d =
          NondeterminismDetectorFactory.getDetector(a);
...
//        }
    ...
    bar.add(new AbstractAction("Run Inputs") {
      public void actionPerformed(ActionEvent e) {
        ...
        if (getObject() instanceof Automaton) {
          Automaton currentAuto =
              (Automaton) getEnvironment().myObjects.get(0);
          ...
          for (int r = 0; r < inputs.length; r++) {
            if (r > 0) {
              if (r % uniqueInputs == 0) {
                currentAuto = (Automaton) getEnvironment().myObjects.get(
                    r / uniqueInputs);
          ...
          BruteParsePane parsePane =
              new BruteParsePane((GrammarEnvironment) getEnvironment(),
                  currentGram, model);
...
//						if(parsePane.stepAction.isEnabled()) model.setResult(r, "Accept", null, getEnvironment().myTransducerStrings, (r%uniqueInputs));
    ...
    if (((InputTableModel) table.getModel()).isMultiple) {
      ...
      bar.add(new AbstractAction("Add input string") {
        public void actionPerformed(ActionEvent arg0) {
          ...
          Object input =
              initialInput((Component) getEnvironment().getActive(),
                  "Input");
          ...
          if (turing) {
            ...
            if (output instanceof String) {
              ...
              getEnvironment().myTestStrings.remove(
                  getEnvironment().myTestStrings.size() - 1);
              return;
            }
          ...
          if (result instanceof String) {
            ...
            getEnvironment().myTestStrings.remove(
                getEnvironment().myTestStrings.size() - 1);
            getEnvironment().myTransducerStrings.remove(
                getEnvironment().myTestStrings.size() - 1);
            return;
          }
      ...
      bar.add(new AbstractAction("Save Results") {
        public void actionPerformed(ActionEvent arg0) {
          ...
          defaultLocation.setActionCommand(
              "Save Results with Original File");
          defaultLocation.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent event) {
            }
          });
          final JRadioButton specifyLocation =
              new JRadioButton("Specify New Location");
              ...
              try {
                ...
                for (int r = 0; r < model.getRowCount(); r++) {
                  ...
                  for (int c = 1; c < model.getColumnCount(); c++) {
                    if ((((String) model.getColumnName(c)).startsWith(
                        "Input")) && !input) {
                      out.write("Input: ");
                      input = true;
                    }
                    if ((((String) model.getColumnName(c)).startsWith(
                        "Output")) && !output && turing) {
                      out.write("Output: ");
                      output = true;
                    }
                    if (((String) model.getColumnName(c)).startsWith(
    ...
    if (finObject instanceof Automaton) {
      ...
      if (getEnvironment().myTestStrings != null &&
          getEnvironment().myTestStrings.size() > 0) bp.inputField.setText(
          (String) getEnvironment().myTestStrings.get(0));
      JSplitPane split = SplitPaneFactory.createSplit(getEnvironment(), true,
          0.5, bp, panel);
   ...
   */
  protected void updateView(String machineFileName, String input,
                            JTableExtender table) {
    ...
    if (current instanceof Automaton &&
        ((InputTableModel) table.getModel()).isMultiple) {
      ...
      newAP.addMouseListener(
          new ArrowDisplayOnlyTool(newAP, newAP.getDrawer()));
      JSplitPane split = SplitPaneFactory.createSplit(getEnvironment(), true,
          0.5, newAP, myPanel);
      MultiplePane mp = new MultiplePane(split);
      ...
      BruteParsePane bp =
          new BruteParsePane((GrammarEnvironment) getEnvironment(), cur,
              null);
      int column = 1;
      if (spot == -1) column = 0;
      bp.inputField.setText((String) table.getModel()
          .getValueAt(table.getSelectedRow(), column));
  ...
  private static Color[] RESULT_COLOR =
