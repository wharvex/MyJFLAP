...
//    }
  ...
  public void mousePressed(MouseEvent event) {
    ...
    if (lastClickedState != null) {
      ...
      if (!lastClickedState.isSelected()) {
        ...
        getView().getDrawer().getAutomaton()
            .selectStatesWithinBounds(bounds);
  ...
  public void mouseDragged(MouseEvent event) {
    ...
    if (lastClickedState != null) {
      ...
      State f = lastClickedTransition.getFromState(), t =
...
//					arrow.setEnd(to);
      ...
      if (f != t) {
        ...
        for (int n = 0; n < trans.length; n++) {
          CurvedArrow arrow =
              (CurvedArrow) getView().getDrawer().transitionToArrowMap.get(
                  trans[n]);
      ...
      bounds =
          new Rectangle(leftX, topY, Math.abs(nowX - initialPointClick.x),
    ...
    if (selectedTransition !=
      ...
      if (transitionInFlux || Math.sqrt(
          (control.getX() - myClickP.x) * (control.getX() - myClickP.x)
              + (control.getY() - myClickP.y) *
              (control.getY() - myClickP.y)) < 15) {
...
//			if (state.getInternalName() != null) {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (item == makeFinal) {
        ...
        EditBlockPane editor = new EditBlockPane(
            ((TMState) state).getInnerTM()); //give it a Turing Machine //just edit the Automaton directly; there is no need for a repaint either, because the other guy does not paint it

        EnvironmentFrame rootFrame =
            parent.getAutomaton().getEnvironmentFrame();
...
//				TMState buffer = ((TuringMachine) getAutomaton()).createTMState((Point)state.getPoint()); //again, we assume that the cast will work, since copyBlock hould never be there except with Turing.
        TMState buffer = ((TuringMachine) getAutomaton()).createTMState(
            new Point(state.getPoint().x + 4,
                state.getPoint().y)); //again, we assume that the cast will work, since copyBlock hould never be there except with Turing.
        buffer.setInnerTM((TuringMachine) ((TMState) state).getInnerTM()
    ...
    private void replaceCharactersInBlock(TMState start, String toReplace,
      ...
      for (int i = 0; i < tm.getStates().length; i++)
        replaceCharactersInBlock((TMState) tm.getStates()[i], toReplace,
            replaceWith);
    ...
    private JMenuItem changeLabel, deleteLabel, deleteAllLabels, editBlock,
        copyBlock, replaceSymbol,
        setName;
  }
